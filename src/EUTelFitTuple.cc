...
void EUTelFitTuple::processEvent(LCEvent * event)
{
	...
	for(int itrack=0; itrack< nTrack ; itrack++)
    {
		...
		if (dutHitFound && telPlanesOk)
		{
			...
			dutTx = atan2(hitBeforeDUTLocal[0]-_fittedXLocal[dutPlaneID], _fittedZLocal[dutPlaneID]-hitBeforeDUTLocal[2]);
			dutTy = atan2(hitBeforeDUTLocal[1]-_fittedYLocal[dutPlaneID], _fittedZLocal[dutPlaneID]-hitBeforeDUTLocal[2]);
			// Fill n-tuple
			int icol=0;
			...
			_FitTuple->fill(icol++,_measuredXLocal[dutPlaneID]);//dutXLocal
			_FitTuple->fill(icol++,_measuredYLocal[dutPlaneID]);//dutYLocal
			_FitTuple->fill(icol++,_measuredZLocal[dutPlaneID]);//dutZLocal
